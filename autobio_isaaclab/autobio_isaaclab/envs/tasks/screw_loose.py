"""Screw-loose task: unscrew a centrifuge tube cap with dual ALOHA arms.

Ports AutoBio's ``ScrewLoose`` task to Isaac Lab DirectRLEnv.

Scene: ``usd_assets/lab_screw_all/lab_screw_all/lab_screw_all.usda``

The agent controls two ALOHA arms (14 DOF total: 7 per arm) to unscrew
the cap of a 50ml centrifuge tube.  Arm1 grips and rotates the cap while
arm2 holds the tube body steady.

Success criteria:
  - Cap-body z-height difference > 0.15
  - Contact between arm1 gripper and cap
  - No contact between nut and bolt (screw fully disengaged)
"""
...
_SCREW_ALL_SCENE_USD = os.path.join(_USD_DIR, "lab_screw_all", "lab_screw_all", "lab_screw_all.usda")
...
def _build_screw_loose_actuators() -> dict:
    """Build per-joint actuator configs for the dual-arm screw-loose scene."""
    actuators = {}
...
def _build_screw_loose_robot_cfg() -> ArticulationCfg:
    """Build ArticulationCfg for the screw-loose scene USD."""
    return ArticulationCfg(
        spawn=sim_utils.UsdFileCfg(
            usd_path=_SCREW_ALL_SCENE_USD,
        ...
        actuators=_build_screw_loose_actuators(),
    )


def _build_rigid_object_cfg(prim_path: str) -> RigidObjectCfg:
    """Build RigidObjectCfg for a rigid body in the screw-loose scene."""
...
# Default robot configuration for the screw-loose scene
SCREW_LOOSE_ROBOT_CFG = _build_screw_loose_robot_cfg()
...
@configclass
class ScrewLooseEnvCfg(AutobioDualEnvCfg):
    """Configuration for the screw-loose task."""

    task_name = "screw_loose"
    episode_length_s = 41.0
    early_stop = True
    ...
    # Success thresholds
    cap_body_z_threshold: float = 0.15  # cap must lift this far above body
    nut_bolt_xy_threshold: float = 0.003  # xy distance to confirm disengagement
    ...
    # Robot: load the screw-loose scene USD (dual-arm robot + table + tube)
    robot_cfg: ArticulationCfg = SCREW_LOOSE_ROBOT_CFG.replace(
...
class ScrewLooseEnv(AutobioDualEnv):
    """Screw-loose task environment.

    The screw-loose scene USD embeds the dual-arm robot, table, and centrifuge
    ...
    cfg: ScrewLooseEnvCfg

    def __init__(self, cfg: ScrewLooseEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _get_rewards(self) -> torch.Tensor:
        """Sparse reward: 1.0 when screw is fully loosened.

        Success requires all three conditions:
          1. Cap-body z-height difference > threshold
          2. Arm1 gripper close to cap (contact proxy)
          3. Cap XY offset from body > threshold (screw threads disengaged)
        """
        cap_pos = self.cap.data.root_pos_w[:, :3]
        body_pos = self.tube_body.data.root_pos_w[:, :3]

        # Condition 1: cap lifted sufficiently above body
        cap_body_z = cap_pos[:, 2] - body_pos[:, 2]
        z_ok = cap_body_z > self.cfg.cap_body_z_threshold

        # Condition 2: arm1 gripper near cap (contact proxy)
        arm1_gripper_pos = self.robot.data.body_pos_w[:, 2, :]  # left_gripper_link
        gripper_cap_dist = torch.norm(arm1_gripper_pos - cap_pos, dim=-1)
        gripper_contact = gripper_cap_dist < 0.05

        # Condition 3: cap XY separated from body (nut-bolt disengaged)
        cap_body_xy = torch.norm(cap_pos[:, :2] - body_pos[:, :2], dim=-1)
        xy_disengaged = cap_body_xy > self.cfg.nut_bolt_xy_threshold

        success = z_ok & gripper_contact & xy_disengaged
